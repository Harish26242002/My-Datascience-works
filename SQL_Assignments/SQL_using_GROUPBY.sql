...
-- SELECT CUST_CITY,count(*)"Total_Cust_more_than 100" From sh.CUSTOMERS group by CUST_CITY HAVING count(*)>100
...
-- SELECT CUST_STATE_PROVINCE,AVG(Cust_credit_limit) from sh.CUSTOMERS group by CUST_STATE_PROVINCE

-- 39. Find states where the total credit limit exceeds 100,000.
-- SELECT CUST_STATE_PROVINCE,AVG(Cust_credit_limit) from sh.CUSTOMERS group by CUST_STATE_PROVINCE HAVING AVG(Cust_credit_limit)>100000

-- 40. Find the maximum credit limit for each income level.
-- SELECT CUST_INCOME_LEVEL,AVG(Cust_credit_limit) FROM SH.CUSTOMERS GROUP BY CUST_INCOME_LEVEL

-- 41. Find income levels where the maximum credit limit is greater than 15,000.
-- SELECT CUST_INCOME_LEVEL,AVG(Cust_credit_limit) FROM SH.CUSTOMERS GROUP BY CUST_INCOME_LEVEL HAVING AVG(Cust_credit_limit)>15000
...
-- SELECT SUBSTR(CUST_EMAIL,INSTR(CUST_EMAIL, '@')+1) AS domain,Count(*)FROM SH.CUSTOMERS GROUP BY SUBSTR(CUST_EMAIL,INSTR(CUST_EMAIL, '@')+1)

-- 47. Find email domains with more than 300 customers.
-- SELECT SUBSTR(CUST_EMAIL,INSTR(CUST_EMAIL, '@')+1) AS domain,Count(*)FROM SH.CUSTOMERS GROUP BY SUBSTR(CUST_EMAIL,INSTR(CUST_EMAIL, '@')+1) having count(*)>300
